...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 firstTrainer, u32 battleTypeFlags)
{
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            u32 fixedOtId = 0;
            u32 ability = 0;
            u32 species = partyData[i].species;

            if (species == SPECIES_BULBASAUR && partyData[i].rival > STARTER_NONE) // this is rival
            {
                species = (partyData[i].rival == STARTER_NAVY) ?
                                        gSaveBlock3Ptr->navyStarters :
                                        gSaveBlock3Ptr->rivalStarters;
            }
